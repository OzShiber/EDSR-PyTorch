...
def gse(x_in, layer, median=True):
    ...
    (This version is fixed to handle non-square inputs)
    """
    ...
    # --- 1. Pad to Square ---
    # Find max dimension and calculate padding
    max_dim = max(h, w)
    pad_h = max_dim - h
    pad_w = max_dim - w
    # F.pad format is (pad_left, pad_right, pad_top, pad_bottom)
    x_in_square = F.pad(x_in, (0, pad_w, 0, pad_h), mode='replicate')

    # --- 2. Add Border Padding (for conv artifacts) ---
    # This is your original padding, now applied to the square tensor
    x_in_padded = F.pad(x_in_square, (1, 1, 1, 1), mode='replicate')
    ...
    # This torch.cat will now work, as all tensors are square
    x_batch = torch.cat(x_list, dim=0)
    y_batch = layer(x_batch)
    y_list = y_batch.chunk(8, dim=0)

    # --- 3. Invert Transformations ---
    y_inv_list = []
    for i in range(8):
        y_aug = y_list[i]
        if i % 2 == 1:
        ...
        # Un-pad the convolution border artifacts
    ...
    # y_agg is [B, C, max_dim, max_dim]
    # Crop the padding we added in step 1
    y_out = y_agg[:, :, :h, :w]
